...
function CCSUILoader:createButton(options)
	local images = self:getButtonStateImages(options)
	local node
	if table.nums(images) == 1 then
		local params = {
				scale9 = options.scale9Enable,
				width = options.width,
				height = options.height,
				}
		node = interface.CButton.new(images.normal, nil, options.tag, params)
		--cc.ui.UIScaleButton.new(images.normal,options.tag)  -- UIScaleButton还没完善，有时间完善后再使用
	else
		node = cc.ui.UIPushButton.new(images,
			{scale9 = options.scale9Enable,
			flipX = options.flipX,
			flipY = options.flipY,
			tag = options.tag})
	...
	if options.text and options.text ~= "" then
		node:setButtonLabel(
			cc.ui.UILabel.new({text = intl.Trans(options.text),
...
function CCSUILoader:createLoadingBar(options)
	...
	local node = cc.ui.UIProcessBar.new(params)
...
function CCSUILoader:createLabel(options)
	local bcolor
	if options.tag == 1 then  -- 这里做是否包边使用
		bcolor = display.COLOR_BLACK_ALPHA
	end
	local node = cc.ui.UILabel.new({text = intl.Trans(options.text),
...
function CCSUILoader:createEditBox(options)
	...
	if true then --  强制生成EditBox
		editBox = cc.ui.UIInput.new({
			UIInputType = 1,
			image = "#frame_barbg.png",
...
function CCSUILoader:createPanel(options)
	...
	if options.backGroundScale9Enable then
		if options.backGroundImageData and options.backGroundImageData.path then
			...
			if 1 == options.backGroundImageData.resourceType then  --self.bUseTexture then 后面是原版本使用的判断接口！！
				UILoaderUtilitys.loadTexture(options.backGroundImageData.plistFile)
