...
import(".ByteArray")
import(".bit")
local ByteArrayVarint = setmetatable({}, {__index=import(".ByteArray")})

function ByteArrayVarint.new(...)
	local obj = setmetatable({}, {__index=ByteArrayVarint})
	obj:ctor(...)
	return obj
end
...
function ByteArrayVarint:_zigZagDecode(__value)
	...
	return bit.bxor(bit.brshift(__value, 1), bit.bnot(0)) - 4294967296
end
...
function ByteArrayVarint:_decodeVarint()
	...
	while self._pos <= #self._buf do
		...
		if __shift >= 32 then
			print("Error ByteArrayVarint:_decodeVarint", "32bit bitwise is unavailable in BitOp!")
			return 0
		end
	end
	return 0
...
function ByteArrayVarint:createSubBuf(iOffset, iLength)
	local subBuf = ByteArrayVarint.new(self._endian)
	local len = #self._buf
	if iOffset <= len then
		local iEnd = iOffset + iLength
		if iEnd > len then
			iEnd = len
		end
		local iCnt = 0
		for i = iOffset, iEnd do
			iCnt = iCnt + 1
			subBuf._buf[iCnt] = self._buf[i]
		end
	end
	return subBuf
end
